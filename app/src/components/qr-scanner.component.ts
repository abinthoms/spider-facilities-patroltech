...
export class QrScannerComponent implements OnDestroy, OnInit {
  ...
  async getCameras() {
    try {
      ...
      this.snackBar.open("Error getting cameras", "Close");
    }
  }
  ...
  async startQrCodeScanner() {
    if (!this.canScan || this.selectedCamera === null) {
      this.snackBar.open("Permissions are required to scan QR codes", "Close", { duration: 5000 });
      return;
    }
    ...
    this.html5QrCode.start(
      ...
      this.snackBar.open("Error starting the QR code scanner", "Close");
    });
  }
  ...
  private onQrCodeDetected(qrCodeMessage: string) {
    ...
    this.readService.saveReadTag(qrCodeMessage).subscribe({
      next: (resp) => {
        ...
        this.snackBar.open("Tag read saved successfully", "Accept");
      },
      error: (err) => {
        console.error("Error saving tag read:", err);
        this.snackBar.open("Error saving tag read", "Accept");
      }
    })
  }
  ...
  async toggleFlash() {
    ...
    try {
      ...
      this.snackBar.open("Could not control the flash", "Close");
    }
  }
